...
const FName AKrazyKartsPawn::LookUpBinding("LookUp");
const FName AKrazyKartsPawn::LookRightBinding("LookRight");

#define LOCTEXT_NAMESPACE "VehiclePawn"

AKrazyKartsPawn::AKrazyKartsPawn()
{
	// Car mesh
	...
	Vehicle4W->WheelSetups[0].WheelClass = UKrazyKartsWheelFront::StaticClass();
	Vehicle4W->WheelSetups[0].BoneName = FName("Wheel_Front_Left");
	Vehicle4W->WheelSetups[0].AdditionalOffset = FVector(0.f, -12.f, 0.f);

	Vehicle4W->WheelSetups[1].WheelClass = UKrazyKartsWheelFront::StaticClass();
	Vehicle4W->WheelSetups[1].BoneName = FName("Wheel_Front_Right");
	Vehicle4W->WheelSetups[1].AdditionalOffset = FVector(0.f, 12.f, 0.f);

	Vehicle4W->WheelSetups[2].WheelClass = UKrazyKartsWheelRear::StaticClass();
	Vehicle4W->WheelSetups[2].BoneName = FName("Wheel_Rear_Left");
	Vehicle4W->WheelSetups[2].AdditionalOffset = FVector(0.f, -12.f, 0.f);

	Vehicle4W->WheelSetups[3].WheelClass = UKrazyKartsWheelRear::StaticClass();
...
void AKrazyKartsPawn::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAxis("MoveForward", this, &AKrazyKartsPawn::MoveForward);
	...
	PlayerInputComponent->BindAction("ResetVR", IE_Pressed, this, &AKrazyKartsPawn::OnResetVR); 
}

void AKrazyKartsPawn::MoveForward(float Val)
{
	GetVehicleMovementComponent()->SetThrottleInput(Val);
}

void AKrazyKartsPawn::MoveRight(float Val)
{
	GetVehicleMovementComponent()->SetSteeringInput(Val);
}

void AKrazyKartsPawn::OnHandbrakePressed()
{
	GetVehicleMovementComponent()->SetHandbrakeInput(true);
}

void AKrazyKartsPawn::OnHandbrakeReleased()
{
	GetVehicleMovementComponent()->SetHandbrakeInput(false);
}

void AKrazyKartsPawn::OnToggleCamera()
{
	EnableIncarView(!bInCarCameraActive);
}

void AKrazyKartsPawn::EnableIncarView(const bool bState, const bool bForce)
{
...
void AKrazyKartsPawn::Tick(float Delta)
{
	Super::Tick(Delta);
...
void AKrazyKartsPawn::SetupInCarHUD()
{
